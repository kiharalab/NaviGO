...
def go_prediction_check(inrec):
    ...
    if len(fields) != 3:
        ...
        return correct, errmsg, f1, f2
    return correct, errmsg, fields[0], fields[1]
...
#        print inline, state
        ...
        if state == "author":
            ...
            correct, errmsg, protein, go = go_prediction_check(inline)
            if protein not in preds:
                preds[protein] = []
            preds[protein].append(go)
            handle_error(correct, errmsg,inline)
            if first_prediction:
                visited_states.append(state)
    ...
    if (visited_states != legal_states1 and
        ...
        for pro in preds:
            print pro + " " + ",".join(preds[pro])+","
def usage():
    print "Usage: cafa_go_format_checker infile"
    
    
def navigo_checker(infile, preds):
    correct = True
    for inline in open(infile,'r'):
        ...
        gos = inrec[1].split(",")
        if len(gos) == 0:
            handle_error(False, "No go preds for protein", inrec)

        if pro not in preds:
            preds[pro] = []
        
        for go in gos:
            if not go_field.match(go):
                continue
            preds[pro].append(go)
        if len(preds[pro]) == 0:
            handle_error(False, "No go preds for protein", inrec)
    ...
    if len(preds.keys()) == 0:
        handle_error(False, "No preds", inrec)
...
if __name__ == '__main__':
    try:
        preds = {}
        cafa_checker(sys.argv[1], preds)
    except:
        preds = {}
        navigo_checker(sys.argv[1], preds)
